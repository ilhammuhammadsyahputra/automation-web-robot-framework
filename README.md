...
**Symptom:** `SessionNotCreatedException: session not created`

**Cause:** The ChromeDriver version does not match the installed Chrome browser version.
...
If the error persists, delete the cached driver and let `webdriver-manager` re-download it:
```bash
rm -rf ~/.wdm
```
...
**Symptom:** The test fails immediately before any browser window appears.

**Fix:** Confirm Google Chrome is installed:
```bash
# macOS
...
**Cause:** The virtual environment is not active, or `pip install` was not run.
...
### `Variable '\${USER_FIRSTNAME}' not found`

**Cause:** A test case that requires a Suite Variable (e.g., `${REGISTERED_EMAIL}`) was run in isolation without the Suite Setup having executed first.

**Fix:** Run the full suite file instead of a single test case tag. The Suite Setup (`Register And Login Before Shopping`) must run before TC-03 or TC-04:
```bash
robot --outputdir reports features/store/shopping.robot
```
...
### Element not found (`ElementNotFound` / timeout)

**Symptom:** A `Wait Until Element Is Visible` step times out after 15 seconds.

**Cause:** The site's UI may have been updated, causing a `data-testid` attribute to change or be removed.

**Fix:**
1. Open the affected page in Chrome.
2. Press `F12` to open DevTools → Elements tab.
3. Search (`Ctrl+F`) for the `data-testid` value from the failing locator.
4. If the attribute has changed, update the corresponding locator in `resources/locators/`.

---

### Test fails at login redirect check

**Symptom:** `Verify Login Redirected To Homepage` fails even though the browser visually redirected.

**Cause:** SeleniumLibrary does not provide a `Location Should Not Contain` keyword. The project uses `Get Location` + Robot Framework's `Should Not Contain` as the correct workaround.

This is already implemented in `resources/keywords/auth_keywords.robot`. If this error occurs, confirm the keyword has not been accidentally replaced with the non-existent `Location Should Not Contain`.
